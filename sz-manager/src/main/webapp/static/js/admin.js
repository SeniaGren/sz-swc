		...
		layer.open({
			...
			success: function(layero, index) {
                ...
                $.each(result, function (i,item) {
                	//[{"uname":"haha","upassword":"123","uid":"4"}]
					if(("uname" in item)){
                        body.find('#userName').val(item.uname);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#password').val(item.upassword);
                        body.find('#uid').val(item.uid);
					}
					// alert(item)
                    if(("aboutname" in item)){
                    // 	alert(item.aboutid)
                        body.find('#aboutid').val(item.aboutid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#aboutname').val(item.aboutname);
                        body.find('#aboutcontent').val(item.aboutcontent);
                    }

                    if(("serviceid" in item)){
                        // 	alert(item.aboutid)
                        body.find('#serviceid').val(item.serviceid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#servicename').val(item.servicename);
                        body.find('#servicecontent').val(item.servicecontent);
                        body.find('#serviceicon').val(item.serviceicon);
                    }

                    if(("pricingid" in item)){
                        // 	alert(item.aboutid)
                        body.find('#pricingid').val(item.pricingid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#pricingname').val(item.pricingname);
                        body.find('#pricingcontent').val(item.pricingcontent);
                        body.find('#pricingprice').val(item.pricingprice);
                        body.find('#pricingselect').val(item.pricingselect);
                    }

                    if(("connectid" in item)){
                        // 	alert(item.aboutid)
                        body.find('#connectid').val(item.connectid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#qqnumber').val(item.qqnumber);
                        body.find('#wechatnumber').val(item.wechatnumber);
                        body.find('#weibonumber').val(item.weibonumber);
                        body.find('#mailnumber').val(item.mailnumber);
                        body.find('#telephone').val(item.telephone);
                    }
                    if(("portfolioid" in item)){
                        // 	alert(item.aboutid)
                        body.find('#portfolioid').val(item.portfolioid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#portfolioname').val(item.portfolioname);
                        body.find('#portfolioimgs').val(item.portfolioimgs);
                        body.find('#portfoliooccupation').val(item.portfoliooccupation);
                    }
                    if(("indexid" in item)){
                        // 	alert(item.aboutid)
                        body.find('#indexid').val(item.indexid);//这里是为动态select赋值，在弹出层创建隐藏元素
                        body.find('#indexcontent').val(item.indexcontent);
                        body.find('#indexname').val(item.indexname);
                    }
